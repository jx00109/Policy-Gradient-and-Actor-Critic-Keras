...
class Agent_PG(Agent):
    def __init__(self, env, args):
        ...
        if not os.path.exists(args.save_summary_path):
            os.mkdir(args.save_summary_path)
        if not os.path.exists(args.save_network_path):
            os.mkdir(args.save_network_path)
        self.env = env
        ...
        if args.test_pg:
            ...
            # Model for Breakout #
            model = Sequential()
            model.add(Conv2D(32, kernel_size=(9, 9), strides=4, activation='relu', input_shape=(80, 80, 1),
                             init='he_uniform'))
